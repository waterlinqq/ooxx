...
// Per team, because the two sides no longer share one lineup.
let searchRosters = { blue: getBoardMode().roster, red: getBoardMode().roster };
...
function inferEnemyComposition(state, enemyTeam = 'blue') {
  const rosterTotals = {};
  for (const classId of searchRosters[enemyTeam] ?? []) {
...
export function chooseAiAction(state, teamOrOptions = 'red') {
  ...
  const sharedRoster = options.roster ?? fallbackMode.roster;
  searchRosters = {
    blue: options.rosters?.blue ?? sharedRoster,
    red: options.rosters?.red ?? sharedRoster,
  };
